            ...
            if count_usize >= PARALLEL_THRESHOLD {
                ...
                dist_slice
                    ...
                    .for_each(|(chunk_idx, chunk)| {
                        ...
                        for (offset, slot) in
                            chunk[remainder_start..chunk_len].iter_mut().enumerate()
                        {
        ...
        || {
            ...
            if !has_transform {
                ...
                for i in remainder_start..count_usize {
                    ...
                    dist_slice[i] =
                        crate::animation::eval_animated_compiled(&comp, params_ref, point);
                }
            }
...
mod tests {
    ...
    /// Every exported function catches panics: a call that trips an internal
    /// panic returns its sentinel and leaves a message for
    /// `alice_sdf_last_error` instead of unwinding into the C caller (which
    /// aborts the host process).
    #[test]
    fn panic_inside_ffi_returns_sentinel_and_message() {
        use super::super::guard::{clear_last_error, take_last_error};
        clear_last_error();
        // Direct guard check with the same sentinel the batch functions use.
        let r = ffi_guard(
            BatchResult {
                count: 0,
                result: SdfResult::Unknown,
            },
            || -> BatchResult { panic!("internal invariant violated") },
        );
        assert_eq!(r.result, SdfResult::Unknown);
        assert_eq!(r.count, 0);
        let msg = take_last_error().expect("panic message recorded");
        assert!(msg.contains("internal invariant violated"), "{msg}");

        // Real exported entry points with hostile input never unwind.
        let node = super::super::primitives::alice_sdf_sphere(1.0);
        let mesh = super::super::mesh::alice_sdf_generate_mesh(node, 0, 1.0);
        // resolution 0: either an empty mesh handle or the sentinel, never an abort
        if !mesh.is_null() {
            super::super::mesh::alice_sdf_free_mesh(mesh);
        }
        let mut dist = [0.0f32; 1];
        let r = unsafe { alice_sdf_eval_batch(node, std::ptr::null(), dist.as_mut_ptr(), 1) };
        assert_ne!(r.result, SdfResult::Ok);
        super::super::memory::alice_sdf_free(node);
        clear_last_error();
    }
