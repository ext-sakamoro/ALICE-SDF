...
// `CompiledSdfField` only holds an `Arc<CompiledSdf>` (Vec fields, no
// interior mutability), so `Send + Sync` are derived automatically — the
// `unsafe impl`s that used to assert this were redundant.
