//! Unified scalar bytecode evaluator (single source of truth)
//!
//! Both [`super::eval::eval_compiled`] (plain bytecode) and
//! [`super::eval_bvh::eval_compiled_bvh`] (BVH-annotated bytecode) execute the
//! same instruction stream with the same semantics, so they share this one
//! stack machine. The `match` over [`OpCode`] is **exhaustive on purpose**:
//! adding an opcode without teaching this evaluator about it is a compile
//! error rather than a silent wrong distance.
//!
//! Historical note (1.9.1): before this module existed the two evaluators were
//! hand-copied and each carried a `_ =>` fallback that evaluated unknown
//! primitives as a unit sphere and unknown modifiers as a no-op.
//!
        ...
        match inst.opcode {
            ...
            OpCode::PopTransform => {
                ...
                match frame.opcode {
                    ...
                    OpCode::Displacement => {
                        ...
                        let d = value_stack[vsp - 1];
                        value_stack[vsp - 1] =
                            modifier_sine_displacement(d, frame.point, amplitude, frequency);
                    }
                    ...
                    OpCode::LatticeDeform => {
                        // Same law as the tree evaluator: eval(child, q) / correction
                        value_stack[vsp - 1] /= frame.params[0];
                    }
            ...
            OpCode::LatticeDeform => {
                coord_stack[csp] = CoordFrame {
                    ...
                    params: [1.0, 0.0, 0.0, 0.0], // params[0] = Lipschitz correction (set below)
                    aux_offset: inst.aux_offset,
                    aux_len: inst.aux_len,
                };
                csp += 1;
                ...
                if inst.aux_len >= 9 {
                    ...
                    coord_stack[csp - 1].params[0] = correction;
                }
            }
            OpCode::SdfSkinning => {
                coord_stack[csp] = CoordFrame {
            ...
            OpCode::ExpSmoothUnion => {
                ...
                let b = value_stack[vsp];
                let a = value_stack[vsp - 1];
                value_stack[vsp - 1] = sdf_exp_smooth_union(a, b, inst.params[0]);
            }
            ...
            OpCode::ExpSmoothIntersection => {
                ...
                let b = value_stack[vsp];
                let a = value_stack[vsp - 1];
                value_stack[vsp - 1] = sdf_exp_smooth_intersection(a, b, inst.params[0]);
            }
            ...
            OpCode::ExpSmoothSubtraction => {
                ...
                let b = value_stack[vsp];
                let a = value_stack[vsp - 1];
                value_stack[vsp - 1] = sdf_exp_smooth_subtraction(a, b, inst.params[0]);
            }

            // === Shear (prefix modifier, paired with PopTransform) ===
            OpCode::Shear => {
                coord_stack[csp] = CoordFrame {
                    point: p,
                    scale_correction,
                ...
                let shear = Vec3::new(inst.params[0], inst.params[1], inst.params[2]);
                p = modifier_shear(p, shear);
            }
            ...
            // with `PopTransform` (same contract as the SIMD evaluator).
            OpCode::Animated => {
                coord_stack[csp] = CoordFrame {
                    point: p,
                    scale_correction,
