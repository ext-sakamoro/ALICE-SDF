    ...
    fn transpile_node_inner(
        ...
        match node {
            ...
            SdfNode::IcosahedralSymmetry { child } => {
                ...
                writeln!(code, "    var {} = abs({});", p_ico, point_var).unwrap();
                ...
                for _ in 0..8 {
                    ...
                    writeln!(code, "    if ({}.x + {} * {}.y < 0.0) {{ {} = vec3<f32>(-{}.x, -{}.y * {}, {}.z); }}",
                        p_ico, phi_s, p_ico, p_ico, p_ico, p_ico, phi_s, p_ico).unwrap();
                    writeln!(code, "    if ({}.y + {} * {}.z < 0.0) {{ {} = vec3<f32>({}.x, -{}.y, -{}.z * {}); }}",
                        p_ico, phi_s, p_ico, p_ico, p_ico, p_ico, p_ico, phi_s).unwrap();
                    writeln!(code, "    if ({}.z + {} * {}.x < 0.0) {{ {} = vec3<f32>(-{}.x * {}, {}.y, -{}.z); }}",
            ...
            SdfNode::SurfaceRoughness {
                ...
                writeln!(code, "    var {} = 0.0;", fbm_var).unwrap();
                writeln!(code, "    {{").unwrap();
                writeln!(code, "        var a = 0.5;").unwrap();
                writeln!(code, "        var q = {} * {};", point_var, freq_s).unwrap();
                writeln!(code, "        for(var i = 0; i < {}; i++) {{", octaves_val).unwrap();
                writeln!(
                    code,
                    "            {} += a * hash_noise_3d(q, u32(i));",
                    fbm_var
                )
                .unwrap();
                ...
                let var = self.next_var();
                writeln!(code, "    let {} = {} + {};", var, d, fbm_var).unwrap();
                var
            }
