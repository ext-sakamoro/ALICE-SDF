    ...
    fn transpile_node_inner(
        ...
        match node {
            ...
            SdfNode::ColumnsUnion { a, b, r, n } => {
                ...
                writeln!(
                    code,
                    "    {v}_ra = fmod({v}_ra + {v}_cs * 0.5, {v}_cs) - {v}_cs * 0.5;",
                    v = var
                )
                .unwrap();
            ...
            SdfNode::Scale { child, factor } => {
                ...
                let new_p = self.next_var();
                writeln!(code, "    float3 {} = {} * {};", new_p, point_var, inv_f_s).unwrap();
            ...
            SdfNode::Displacement { child, strength } => {
                ...
                .unwrap();
                writeln!(code, "    float {} = {} + {} * {};", var, d, disp_var, s).unwrap();
                var
            }
            ...
            SdfNode::SweepBezier { child, p0, p1, p2 } => {
                ...
                writeln!(
                    code,
                    "    float3 {} = float3(sqrt({np}_dx*{np}_dx + {np}_dz*{np}_dz), {p}.y, 0.0);",
                    new_p,
                    np = new_p,
                    p = point_var
                )
            ...
            SdfNode::Segment2D {
                ...
                writeln!(
                    code,
                    "    float2 {v}_pa = {p}.xy - float2({ax}, {ay});",
                    v = v,
                    p = point_var,
                    ax = ax,
                    ay = ay
                ...
                writeln!(
                    code,
                    "    float2 {v}_ba = float2({bx}, {by}) - float2({ax}, {ay});",
                    v = v,
                    bx = bx,
                    by = by,
                    ax = ax,
                ...
                writeln!(
                    code,
                    "    float {v}_h = saturate(dot({v}_pa, {v}_ba) / dot({v}_ba, {v}_ba));",
                    v = v
                )
                .unwrap();
                writeln!(
