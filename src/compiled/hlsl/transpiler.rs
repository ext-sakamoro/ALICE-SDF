    ...
    fn transpile_node_inner(
        ...
        match node {
            ...
            SdfNode::BoxFrame { half_extents, edge } => {
                ...
                let var = self.next_var();
                writeln!(code, "    float3 {} = abs({}) - float3({}, {}, {});", pv, point_var, bx, by, bz).unwrap();
                writeln!(code, "    float3 {} = abs({} + {}) - {};", qv, pv, e, e).unwrap();
                writeln!(code, "    float {} = length(max(float3({}.x, {}.y, {}.z), 0.0)) + min(max({}.x, max({}.y, {}.z)), 0.0);", d1, pv, qv, qv, pv, qv, qv).unwrap();
                writeln!(code, "    float {} = length(max(float3({}.x, {}.y, {}.z), 0.0)) + min(max({}.x, max({}.y, {}.z)), 0.0);", d2, qv, pv, qv, qv, pv, qv).unwrap();
                writeln!(code, "    float {} = length(max(float3({}.x, {}.y, {}.z), 0.0)) + min(max({}.x, max({}.y, {}.z)), 0.0);", d3, qv, qv, pv, qv, qv, pv).unwrap();
            ...
            SdfNode::DiamondSurface { scale, thickness } => {
                ...
                let var = self.next_var();
                writeln!(code, "    float3 {} = {} * {};", sp, point_var, sc).unwrap();
            ...
            SdfNode::Morph { a, b, t } => {
                ...
                writeln!(code, "    float {} = lerp({}, {}, {});", var, d_a, d_b, t_s).unwrap();
                var
            }
            ...
            SdfNode::Pipe { a, b, r } => {
                ...
                let var = self.next_var();
                let r_s = self.param(*r);
                writeln!(code, "    float {} = length(float2({}, {})) - {};", var, d_a, d_b, r_s).unwrap();
                var
            }
            ...
            SdfNode::Rect2D { half_extents, half_height } => {
                ...
                writeln!(code, "    float {v}_d2d = length(max(float2({v}_dx, {v}_dy), 0.0)) + min(max({v}_dx, {v}_dy), 0.0);", v=v).unwrap();
                writeln!(code, "    float {v}_dz = abs({p}.z) - {hh};", v=v, p=point_var, hh=hh).unwrap();
                writeln!(code, "    float {v}_wx = max({v}_d2d, 0.0);", v=v).unwrap();
                writeln!(code, "    float {v}_wy = max({v}_dz, 0.0);", v=v).unwrap();
                writeln!(code, "    float {v} = sqrt({v}_wx*{v}_wx + {v}_wy*{v}_wy) + min(max({v}_d2d, {v}_dz), 0.0);", v=v).unwrap();
            ...
            SdfNode::Segment2D { a, b, thickness, half_height } => {
                ...
                let hh = self.param(*half_height);
                let v = self.next_var();
                writeln!(code, "    float2 {v}_pa = {p}.xy - float2({ax}, {ay});", v=v, p=point_var, ax=ax, ay=ay).unwrap();
                writeln!(code, "    float2 {v}_ba = float2({bx}, {by}) - float2({ax}, {ay});", v=v, bx=bx, by=by, ax=ax, ay=ay).unwrap();
                writeln!(code, "    float {v}_h = saturate(dot({v}_pa, {v}_ba) / dot({v}_ba, {v}_ba));", v=v).unwrap();
            ...
            SdfNode::RoundedRect2D { half_extents, round_radius, half_height } => {
                ...
                writeln!(code, "    float {v}_d2d = length(max(float2({v}_dx, {v}_dy), 0.0)) + min(max({v}_dx, {v}_dy), 0.0) - {rr};", v=v, rr=rr).unwrap();
                writeln!(code, "    float {v}_dz = abs({p}.z) - {hh};", v=v, p=point_var, hh=hh).unwrap();
                writeln!(code, "    float {v}_wx = max({v}_d2d, 0.0);", v=v).unwrap();
                writeln!(code, "    float {v}_wy = max({v}_dz, 0.0);", v=v).unwrap();
                writeln!(code, "    float {v} = sqrt({v}_wx*{v}_wx + {v}_wy*{v}_wy) + min(max({v}_d2d, {v}_dz), 0.0);", v=v).unwrap();
            ...
            SdfNode::Shear { child, shear } => {
                ...
                let new_p = self.next_var();
                writeln!(code, "    float3 {np} = float3({p}.x, {p}.y - {sx} * {p}.x, {p}.z - {sy} * {p}.x - {sz} * {p}.y);",
    ...
    fn test_transpile_all_variants_exhaustive() {
        ...
        for (name, prim) in &primitives {
            ...
            assert!(
                ...
                "HLSL transpile failed for primitive '{}': missing sdf_eval", name,
            );
        }
        ...
        for (name, op) in &operations {
            ...
            assert!(
                shader.source.contains("float sdf_eval(float3 p)"),
                "HLSL transpile failed for operation '{}': missing sdf_eval", name,
            );
        }
        ...
        for (name, m) in &modifiers {
            ...
            assert!(
                shader.source.contains("float sdf_eval(float3 p)"),
                "HLSL transpile failed for modifier '{}': missing sdf_eval", name,
            );
        }
    }
}
