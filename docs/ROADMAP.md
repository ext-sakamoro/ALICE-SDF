...
| SDF-R2-4 `eval_compiled` scalar 1.95× slower than the tree walker | 🟡 1.12.0 / 2.1.0 | Zeroing three stacks per call (~21 ns) → `MaybeUninit` slots (sphere 30 → 4 ns); the 9/16 self-review still measured 1.3× on a CSG scene → 2.1.0 slims the transform frame (point + instruction index, 4 stores instead of 10): 1.0–1.2× on 5–20 primitive CSG, 0.9× at 40. The remaining gap is the push / pop pair per transform against the tree's single recursion; the scalar VM exists for the SIMD batch path (`eval_compiled_batch_simd`, 4.4× the tree), single-point callers can use either |
