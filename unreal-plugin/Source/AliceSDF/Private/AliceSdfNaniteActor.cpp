...
UStaticMesh* AAliceSdfNaniteActor::CreateNaniteStaticMesh()
{
	...
	// Source models / CreateMeshDescription / NaniteSettings / Build() are
	// WITH_EDITOR only. A cooked game fills a local FMeshDescription and
	// hands it to BuildFromMeshDescriptions (render data only, no Nanite).
#if WITH_EDITOR
	StaticMesh->SetNumSourceModels(1);
	FMeshDescription* MeshDesc = StaticMesh->CreateMeshDescription(0);
#else
	FMeshDescription RuntimeMeshDesc;
	FMeshDescription* MeshDesc = &RuntimeMeshDesc;
#endif
...
#else
	// --- Step 4-6 (cooked game): Nanite data can only be generated by the
	// editor build pipeline, so at runtime the SDF surface becomes a regular
	// static mesh. bFastBuild is the only mode available outside the editor.
	...
	if (!StaticMesh->BuildFromMeshDescriptions({ MeshDesc }, BuildParams))
	{
		UE_LOG(LogTemp, Error, TEXT("ALICE-SDF: BuildFromMeshDescriptions failed for %s"), *GetName());
		PlatformFile.DeleteFile(*ObjPath);
		return nullptr;
	}
	UE_LOG(LogTemp, Log, TEXT("ALICE-SDF: %s built as a regular static mesh (Nanite needs the editor build pipeline)"), *GetName());
#endif
