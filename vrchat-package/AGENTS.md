...
| 4 | Generate the scene | Menu **ALICE-SDF > Generate Sample Scenes**. Headless: see below. It sizes the bounding cube (Mochi: centre (0, 1, 0), scale (4, 2, 4)), assigns the material and adds `SampleMochi_Collider` (UdonSharp creates the backing UdonBehaviour). | `Assets/AliceSDF_SampleScenes/SDF_Mochi.unity` exists; Editor.log contains `[ALICE-SDF] Created scene: Assets/AliceSDF_SampleScenes/SDF_Mochi.unity` and **no** `[ALICE-SDF] AliceSDF.Samples.SampleMochi_Collider not found` |
| 5 | Play in the Editor | Open `SDF_Mochi.unity`, press Play. With the VRChat SDK's ClientSim a local player spawns; on desktop, hold left click on a mochi to grab it, right click while holding to split. Set `Log Events` on the collider first. | Five mochis on a plane are visible; Console / Editor.log shows `[Mochi] grab …` after a click on a mochi, `[Mochi] click miss` after a click on empty space |
...
imported scripts compile between them; exit code 1 on failure):
...
| T5 | After updating the package the Mochi behaves like the old version | Package Manager never overwrites `Assets/Samples/ALICE-SDF for VRChat/<old version>/` | Delete that folder, re-import (§1 step 3), then regenerate the scene: `Samples~` carries no `.meta`, so a re-import assigns new GUIDs and a scene generated before it points at a shader that no longer exists (pink material) |
...
| T10 | Desktop click never grabs (`[Mochi] click miss` every time) | The click is evaluated on the view ray; the ray misses every mochi or the hold is shorter than `Grab Dwell Time` (0.08 s) | Aim at the mochi's centre and hold; `[Mochi] click hit` then `grab` after the dwell |
