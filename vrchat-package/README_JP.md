...
**デスクトップ:** 餅の上で Use（左クリック）を押し続けると、視線上で餅の中心に最も近い点が仮想の右手になり、上記の掴む / 移動 / 分裂 / リリース / 合体が視点移動で動きます。ボタンを離すと落とします。Grab ボタン（右クリック、VR はグリップ）で持っている餅を割ります。Log Events を on にすると、離した時に「持ったまま割ったか / どこまで運んだか」が 1 行出ます。
