...
namespace AliceSDF.Editor
{
    public static class KitProductBuilder
    {
        ...
        private static PrefabDef Mochi(string preset, Color mochi, Color highlight, Color ground, Color detail,
                                       ...
                                       string mochiTex = null, float mochiTexScale = 1f, string groundTex = null, float groundTexScale = 0.5f)
        {
            return new PrefabDef
            {
                ...
                preset = (mat, so) =>
                {
                    ...
                    if (mochiTex != null)
                    {
                        var t = AssetDatabase.LoadAssetAtPath<Texture2D>(Out + "/Textures/" + mochiTex);
                        so.FindProperty("mochiTexture").objectReferenceValue = t;
                        so.FindProperty("mochiTextureScale").floatValue = mochiTexScale;
                        so.FindProperty("mochiTextureStrength").floatValue = 1f;
                        mat.SetTexture("_MochiTex", t); mat.SetFloat("_MochiTexScale", mochiTexScale); mat.SetFloat("_MochiTexStrength", 1f);
                    }
                    if (groundTex != null)
                    {
                        var t = AssetDatabase.LoadAssetAtPath<Texture2D>(Out + "/Textures/" + groundTex);
                        so.FindProperty("groundTexture").objectReferenceValue = t;
                        so.FindProperty("groundTextureScale").floatValue = groundTexScale;
                        so.FindProperty("groundTextureStrength").floatValue = 0.8f;
                        mat.SetTexture("_GroundTex", t); mat.SetFloat("_GroundTexScale", groundTexScale); mat.SetFloat("_GroundTexStrength", 0.8f);
                    }
                }
            };
        }
        ...
        private static readonly PrefabDef[] Prefabs =
        {
            ...
            // Textured preset: a sesame-speckled mochi on a woven mat (both images generated by EnsureTextures, tileable)
            Mochi("Sesame", new Color(0.93f, 0.86f, 0.72f), new Color(0.98f, 0.94f, 0.85f), new Color(0.62f, 0.55f, 0.38f), new Color(0.55f, 0.48f, 0.32f), 0.5f, 0.15f, 1000f, 3f,
                  "MochiSesame.png", 1.0f, "GroundWeave.png", 0.5f),
        ...
        public static string BuildCore()
        {
            ...
            foreach (var d in new[] { Out, Out + "/Scripts", Out + "/Shaders", Out + "/Materials", Out + "/Prefabs", Out + "/Programs", Out + "/Textures" })
        ...
        // Tileable 256 x 256 images for the textured preset, written once
        // (deterministic, so a rebuild does not churn the asset)
        private static void EnsureTextures()
        {
            EnsureTexture("MochiSesame.png", (x, y, rng) =>
            {
                // cream skin with dark sesame seeds: ~90 seeds, each an ellipse, wrapped at the edges
                return Color.clear;   // replaced below (seed map painted in EnsureSeeds)
            }, true);
            EnsureTexture("GroundWeave.png", (x, y, rng) =>
            {
                // woven mat: two interleaved stripe directions with a slight tone jitter
                int cell = 16;
                bool over = ((x / cell) + (y / cell)) % 2 == 0;
                int u = over ? (y % cell) : (x % cell);
                float ridge = 0.85f + 0.15f * Mathf.Sin(u / (float)cell * Mathf.PI);
                float tone = 0.62f * ridge + 0.03f * (float)rng.NextDouble();
                return new Color(tone * 1.05f, tone * 0.95f, tone * 0.7f, 1f);
            }, false);
        }

        private static void EnsureTexture(string name, System.Func<int, int, System.Random, Color> pixel, bool seeds)
        {
            string path = Out + "/Textures/" + name;
            ...
            if (seeds)
            {
                var cream = new Color(0.95f, 0.90f, 0.80f, 1f);
                ...
                for (int s = 0; s < 90; s++)
                {
                    ...
                    float ra = 4.5f + (float)rng.NextDouble() * 1.5f, rb = 2.5f + (float)rng.NextDouble();
                    var seed = new Color(0.12f + 0.05f * (float)rng.NextDouble(), 0.08f, 0.05f, 1f);
                    for (int y = 0; y < N; y++) for (int x = 0; x < N; x++)
                    {
                        // wrapped offset so seeds crossing the edge continue on the other side
                        float dx = x - cx; if (dx > N / 2) dx -= N; if (dx < -N / 2) dx += N;
                        float dy = y - cy; if (dy > N / 2) dy -= N; if (dy < -N / 2) dy += N;
                        float u = (dx * ca + dy * sa) / ra, v = (-dx * sa + dy * ca) / rb;
                        float d = u * u + v * v;
                        if (d < 1f) px[y * N + x] = Color.Lerp(seed, px[y * N + x], Mathf.SmoothStep(0f, 1f, (d - 0.7f) / 0.3f));
                    }
                }
            }
            else
            {
                for (int y = 0; y < N; y++) for (int x = 0; x < N; x++) px[y * N + x] = pixel(x, y, rng);
            }
            tex.SetPixels(px);
            tex.Apply();
...
|---|---|---|
| AliceSDF Mochi (Mochi / Slime / Water / Sesame) | soft blobs on a ground patch, four presets (Sesame shows the texture slots: a seeded skin on a woven mat, both tileable images in Textures/) | grab: hand in / hold left click; split: grip / right click; merge: push together; walk in: your body dents them |
| AliceSDF Wall | a wall that dents where it is hit and heals | punch: hand / hold left click; walk into it |
| AliceSDF Terrain | ground you can build up and dig block by block (1 m cubes on a grid, Minecraft-style; Block Brush off = spheres), and stand on | left hand / left click builds, right hand / right click digs; walk on what you made |
...
Keep a floor collider under Mochi and Wall (their ground is drawn, not walked on). **Do not put a floor collider under a Terrain**: its terrain is the floor (a mesh floor there keeps you from falling into the holes you dig).
...
Terrain — block size (Sculpt Radius = half-side) / smoothness / capacity / Block Brush; Decor — the shape parameters. `Log Events` prints one line per event to the client log.
...
|---|---|---|
| AliceSDF Mochi (Mochi / Slime / Water / Sesame) | 地面パッチの上のやわらかい塊、preset 4 種 (Sesame はテクスチャ枠の見本: 胡麻の皮 + 織り目の敷物、`Textures/` のタイル画像 2 枚) | 掴む: 手を入れる / 左クリック押しっぱなし、分裂: グリップ / 右クリック、合体: 押し合わせる、体当たり: 体の形に凹む |
| AliceSDF Wall | 殴ると凹んで戻る壁 | 殴る: 手 / 左クリック押しっぱなし、歩いて押す |
| AliceSDF Terrain | 1 m のブロック単位で盛って掘れて (Minecraft 方式、Block Brush off で球)、その上に立てる地面 | 左手 / 左クリックで盛る、右手 / 右クリックで掘る、作った地形の上を歩く |
...
Mochi と Wall の地面は「描いているだけ」なので床 collider をその高さに置いてください **Terrain の下には床 collider を置かない**でください (地形自身が足場、床 mesh があると掘った穴に落ちられません)
...
Terrain — ブロックの大きさ (Sculpt Radius = 半辺) / 滑らかさ / 容量 / Block Brush、Decor — 形のパラメータ `Log Events` を on にすると client log にイベントごと 1 行出ます
